...
sys.path.append(os.path.join(BASE_DIR, "amostras"))
from generate_bolas_dataset import build_dataset
...
parser = argparse.ArgumentParser(
    description="Detector GA+RNA de bolas pretas (single/multi) com busca sequencial."
)
...
# Parada por IoU de máscara preta
MASK_IOU_STOP_ENABLE = True
MASK_IOU_STOP_THR    = 0.940
...
# Anti-travamento em quadros totalmente brancos
...
# Crescimento do raio quando só se vê branco por muito tempo
...
# Métricas/avaliação de IoU com GT
IOU_EVAL_STOP      = True
IOU_EVAL_THRESH    = 1.0
...
# Encolhimento mais rápido
SHRINK_GAIN_MIN      = 1.25
SHRINK_GAIN_MAX      = 4.0
SHRINK_CAP_NEAR_PX   = 8
SHRINK_CAP_FAR_PX    = 14

# Permitimos círculos parcialmente fora da imagem
...
_p = PRESETS.get(SEARCH_BUDGET, PRESETS["thorough"])
...
# ============================================================
# FAST MODE – acelera GA e controlador, sem mexer em GIFs
# ============================================================
FAST_MODE = True  # Para rodar razoavelmente rápido
...
if FAST_MODE:
    ...
    SEARCH_BUDGET = "fast"
    _p = PRESETS.get(SEARCH_BUDGET, PRESETS["fast"])
    CTRL_STEPS_COARSE = _p["CTRL_STEPS_COARSE"]
    CTRL_STEPS_FINE   = _p["CTRL_STEPS_FINE"]
    PATIENCE_STEPS    = _p["PATIENCE_STEPS"]
    EXTRA_STEPS_CAP   = _p["EXTRA_STEPS_CAP"]

    EXTRA_STEPS_CAP_TRACE = 400
...
# ============================================================
# 4) Probes auxiliares + função de perda
# ============================================================
...
# ============================================================
# 5) MLP (Rede Neural Artificial) – hidden menor para acelerar
# ============================================================
class NeuralNetwork:
    ...
    def set_weights(self, vector):
        ...
        for i in range(len(self.weights)):
            ...
            self.weights[i] = vector[idx:idx + n_w].reshape(w_shape).astype(np.float32)
            idx += n_w
            n_b = int(np.prod(b_shape))
            self.biases[i] = vector[idx:idx + n_b].astype(np.float32)
            idx += n_b

# ============================================================
# A RNA emite 22 bits de ação
# ============================================================
ACTION_BITS = 22
...
def border_black_direction(img255, cx, cy, r, cos_tab, sin_tab):
    ...
    vx = float(np.mean(xb - cx))
    vy = float(np.mean(yb - cy))
...
# ============================================================
# 9) Controlador (modo sem traço/GIF)
# ============================================================
def run_controller(nn, img255, img_small_bin, steps, cos_tab, sin_tab, metrics_loss_fn, probe_r_list,
    ...
    while t < max_steps:
        ...
        if all_white:
            if sx == 0 and sy == 0:
                sx, sy = scan_dirs[scan_k % 4]
                scan_k += 1
            if (sx > 0 and cx >= size-1-r) or (sx < 0 and cx <= r):
                sx = -sx
            if (sy > 0 and cy >= size-1-r) or (sy < 0 and cy <= r):
                sy = -sy
            ...
            if white_streak % max(1, WHITE_SUPERJUMP_EVERY) == 0:
                tx = int(rng.integers(r, size - r))
                ty = int(rng.integers(r, size - r))
                dx_f = float(tx - cx)
                dy_f = float(ty - cy)
        ...
        if abs(raw_dx) <= DEADZONE_PX:
            raw_dx = 0.0
        if abs(raw_dy) <= DEADZONE_PX:
            raw_dy = 0.0
        ...
        if (l + IMPROVE_EPS) < best[0]:
            ...
            if t >= WARMUP_STEPS:
                no_improve += 1
                if EARLY_STOP and (no_improve >= PATIENCE_STEPS):
                    break
...
# ============================================================
# 10) Controlador com tracing (gera GIF)
# ============================================================
...
def save_gif_for_trace(img255, trace, gt_tuple, out_path, trim_at_stop=True):
    if trim_at_stop and TRIM_GIF_AT_STOP:
...
def save_gif_for_frames(frames_info, gt_tuple, out_path):
    """
    Helper para gerar UM GIF por imagem a partir de vários traces.
    frames_info: lista de (img255, p_dict).
    """
    if not frames_info:
        return
...
CHECKPOINT_TAG   = f"v13_fast_smallnet_iouchase_borderaware_initpolicy_{INIT_POLICY}_inithead_{int(USE_INIT_HEAD)}"
...
def _arch_signature(num_weights, num_bits):
    return {
        ...
        "tag": CHECKPOINT_TAG,
        "version": f"2025-10-30.v13_fast_smallnet",
    }
    ...
    try:
        ...
        if meta_json is not None:
            try:
                ...
                if meta.get("tag") != CHECKPOINT_TAG:
                    return None, None
...
# ============================================================
# 13) Regenera dataset sintético a cada execução
# ============================================================
build_dataset(
    out_dir=Path(DATA_ROOT),
    size=IMG_SIZE,             # 255
    n_single=100,              # quantas imagens "single"
    n_multi=100,               # quantas imagens "multi"
    r_min=8,
    r_max=40,
    k_min_multi=2,
    k_max_multi=6,
    allow_overlap=False,        # igual ao exemplo do gerador
    min_gap=0.0,
    seed=None,                 # None => diferente a cada execução
)

# ============================================================
# 13b) Carrega anotações (dataset) recém-gerado
# ============================================================
...
_num_bits    = int(_num_weights * 16)  # 16 bits por peso
...
for ann in annotations:
    ...
    # ============================================================
    # 15) LOOP DE DETECÇÃO SEQUENCIAL DE MÚLTIPLAS BOLAS
    #     REGRA DO GIF (igual ao locate_balls.py):
    #       - Se achar >= 1 bola:
    #           * mostra todas as tentativas aceitas (trace aparado + erase)
    #           * mostra também a ÚLTIMA tentativa falha
    #       - Se não achar nenhuma bola:
    #           * mostra TODAS as tentativas (todas as MISS)
    # ============================================================
    current_img_full = img_full.copy()
    balls_found = []

    # Todas as tentativas (para o caso de não achar NENHUMA bola)
    all_attempts_frames = []

    # Só as tentativas que viraram bola aceita (trace aparado + erase)
    accepted_frames = []

    # Última tentativa falha depois da última bola aceita
    last_failed_attempt_frames = None

    bad_candidates_in_a_row = 0  # candidatas ruins seguidas
    ...
    while check_for_black(current_img_full):
        ...
        # Chute inicial usando a mesma política global (ex.: random_then_nn)
        cx_start, cy_start, r_start = choose_initial_state(
            nn,
            current_img_full,
            current_img_small,
            size,
        ...
        # Snapshot da imagem ANTES de apagar bola desta tentativa
        ...
        # 1) Registrar TODA a tentativa em all_attempts_frames
        attempt_frames = []
        for p in trace:
            attempt_frames.append((img_for_gif.copy(), p))
        all_attempts_frames.extend(attempt_frames)

        # 2) Verificar se esta tentativa é uma bola válida
        ...
        if not is_valid_ball:
            ...
            # esta tentativa passa a ser a "última falha" por enquanto
            ...
            # não apaga nada, volta pro while para tentar outra bola
            continue

        # 3) Bola foi aceita aqui
        #    - zera contador de candidatas ruins
        #    - limpa "última falha" (porque agora temos uma bola nova aceita)
        #    - adiciona ao GIF apenas o trace aparado + frame de erase
        bad_candidates_in_a_row = 0
        balls_found.append({"x": cx_pred, "y": cy_pred, "r": r_pred, "loss": final_loss})

        # nova bola aceita => "última falha" não é mais relevante
        last_failed_attempt_frames = None

        # 3.1) Trace aparado desta tentativa (até o stop / snap_refine)
        trace_trim = trim_trace_on_stop(trace, keep_snap=KEEP_SNAP_AFTER_STOP)
        for p in trace_trim:
            accepted_frames.append((img_for_gif.copy(), p))

        # 3.2) Apaga a bola encontrada (mascara para branco) para próxima busca
        current_img_full = mask_circle_in_image(current_img_full, cx_pred, cy_pred, r_pred)

        # 3.3) Adiciona um frame extra mostrando a imagem já sem essa bola
        erase_state = dict(best)
        erase_state["t"] = erase_state.get("t", 0) + 1
        erase_state["mode"] = str(erase_state.get("mode", "")) + "|erase"
        accepted_frames.append((current_img_full.copy(), erase_state))

    # ============================================================
    # 16) Gera UM GIF por imagem (se habilitado)
    #      - Se houver bolas aceitas:
    #           usa accepted_frames + última tentativa falha (se existir)
    #      - Se não houver nenhuma bola aceita:
    #           usa all_attempts_frames (todas as tentativas falhas)
    # ============================================================
    if MAKE_GIFS and (GIF_LIMIT is None or gif_count < GIF_LIMIT):
        if len(balls_found) > 0:
            frames_to_use = list(accepted_frames)
            if last_failed_attempt_frames is not None:
                frames_to_use.extend(last_failed_attempt_frames)
        else:
            frames_to_use = list(all_attempts_frames)
        ...
        if len(frames_to_use) > 0:
            ...
            gif_count += 1
            print(f"  [gif] GIF único por imagem salvo em: {gif_path}")
